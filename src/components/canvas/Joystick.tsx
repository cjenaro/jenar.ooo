...
export default function Joystick({ material, whiteMaterial, stickGeometry, whiteGeometry }) {
    ...
    const { rotation: springRotation } = useSpring<{ rotation: [x: number, y: number, z: number]}>({ rotation: directions[direction]});
 ...
 return (
    <animated.group position={[-1.077, -0.664, 0.514]} rotation={springRotation} >
        <mesh
            castShadow
            receiveShadow
